...
@section('header-title', 'Edit Pendidikan')
...
@section('content')
  <div class="max-w-4xl mx-auto space-y-6" x-cloak x-data='educationForm(@json([
    'institution' => old('institution', $education->institution),
    'degree' => old('degree', $education->degree),
    'startDate' => old('start_date', $education->start_date),
    'endDate' => old('end_date', $education->end_date),
    'isCurrent' => old('end_date') === '' && old('start_date') !== null
      ? true
      : (old('end_date') !== null
        ? false
        : in_array(strtolower((string) $education->end_date), ['sekarang', 'present', 'now', 'ongoing', ''])),
    'sortOrder' => old('sort_order', $education->sort_order),
    ...
    <div class="anim-in relative bg-gradient-to-r from-emerald-500 via-teal-500 to-sky-500 rounded-2xl p-7 sm:p-8 overflow-hidden shadow-lg shadow-emerald-600/20">
      ...
      <div class="relative flex items-center gap-4">
        ...
        <div>
          <h2 class="text-lg font-extrabold text-white" x-text="institution || 'Edit Pendidikan'"></h2>
          <p class="text-sm text-emerald-50 mt-0.5">Perbarui riwayat studi, pratinjau langsung di kanan.</p>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-6 items-start">
      {{-- Form --}}
      <form method="POST" action="{{ route('admin.educations.update', $education) }}" x-on:submit="submitting = true" class="xl:col-span-3 space-y-5">
        @csrf
        @method('PUT')
