...
@section('header-title', 'Edit Proyek')
...
@push('styles')
  <style>
    ...
    .field-card { transition: box-shadow .25s, border-color .25s; }
...
@section('content')
  @php
    $initial = [
      ...
      'sortOrder' => (string) old('sort_order', $project->sort_order),
      'isActive' => (bool) old('is_active', $project->is_active),
      'title' => old('title', $project->title),
      'subtitle' => old('subtitle', $project->subtitle),
      'description' => old('description', $project->description),
      'thumbnail' => $project->thumbnail ? asset_url($project->thumbnail) : null,
    ];
  @endphp

  <div class="max-w-4xl mx-auto space-y-6" x-cloak x-data='projectForm(@json($initial, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_TAG))'>
    ...
    <div class="anim-in relative bg-gradient-to-r from-indigo-600 via-indigo-500 to-violet-600 rounded-2xl p-7 sm:p-8 overflow-hidden shadow-lg shadow-indigo-600/20">
      ...
      <div class="relative flex items-center gap-4">
        ...
        <div>
          <h2 class="text-lg font-extrabold text-white" x-text="'"' + title + '"' || title"></h2>
          <p class="text-sm text-indigo-100 mt-0.5">Perbarui informasi proyek, pratinjau langsung di panel kanan.</p>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-6 items-start">
      {{-- Form --}}
      <form method="POST" action="{{ route('admin.projects.update', $project) }}" enctype="multipart/form-data" x-on:submit="submitting = true" class="xl:col-span-3 space-y-5">
        @csrf
        @method('PUT')
          ...
          <div class="p-6 space-y-5">
            ...
            <div>
              ...
              <label for="thumb-input" class="group field-card cursor-pointer relative block rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/60 hover:border-indigo-300 hover:bg-indigo-50/40 transition-all duration-300 overflow-hidden">
                ...
                <template x-if="!thumbnailPreview">
                  <div class="relative aspect-video flex items-center justify-center bg-gray-100">
                    <template x-if="thumbnail">
                      <img :src="thumbnail" class="absolute inset-0 w-full h-full object-cover" alt="Thumbnail saat ini">
                    </template>
                    <div class="absolute inset-0 bg-black/0 hover:bg-black/40 transition-colors duration-300 flex items-center justify-center">
                      <div class="w-12 h-12 rounded-xl bg-white/90 shadow flex items-center justify-center text-indigo-500 group-hover:scale-110 group-hover:-rotate-6 transition-transform duration-300 animate-floaty">
                    ...
                    <span class="absolute bottom-2 right-2 text-[10px] font-bold uppercase tracking-wide bg-black/50 text-white px-2 py-1 rounded-md backdrop-blur">Klik untuk ganti</span>
                  </div>
                </template>
                <template x-if="thumbnailPreview">
            ...
            <div class="flex items-center justify-between p-4 rounded-2xl border transition-all duration-300"
              ...
              <label class="relative inline-flex items-center cursor-pointer shrink-0">
                ...
                <div class="w-12 h-7 rounded-full transition-all duration-300 peer-focus:ring-4 peer-focus:ring-indigo-500/20" :class="isActive ? 'bg-emerald-500' : 'bg-gray-300'"></div>
          ...
          <div class="p-5">
            <div class="rounded-2xl border border-gray-100 shadow-sm overflow-hidden group transition-all duration-300 hover:shadow-md">
              <div class="relative aspect-video bg-gradient-to-br from-indigo-100 to-violet-100">
                ...
                <template x-if="!thumbnailPreview && thumbnail">
                  <img :src="thumbnail" class="w-full h-full object-cover" alt="">
                </template>
                <div x-show="!thumbnailPreview && !thumbnail" class="w-full h-full flex items-center justify-center text-indigo-300" x-cloak>
