...
@section('header-title', 'Tambah Skill Baru')
...
@push('styles')
  <style>
    ...
    .animate-pop { animation: popS .45s cubic-bezier(.22, 1, .36, 1) both; }
...
@section('content')
  @php
    $cats = [
      'marketplace' => ['label' => 'Marketplace', 'chip' => 'bg-amber-50 text-amber-700 border-amber-200', 'active' => 'bg-amber-600 text-white border-amber-600 shadow-amber-600/25', 'dot' => 'bg-amber-500'],
      'uiux' => ['label' => 'UI/UX', 'chip' => 'bg-indigo-50 text-indigo-700 border-indigo-200', 'active' => 'bg-indigo-600 text-white border-indigo-600 shadow-indigo-600/25', 'dot' => 'bg-indigo-500'],
      'data' => ['label' => 'Data', 'chip' => 'bg-emerald-50 text-emerald-700 border-emerald-200', 'active' => 'bg-emerald-600 text-white border-emerald-600 shadow-emerald-600/25', 'dot' => 'bg-emerald-500'],
      'technical' => ['label' => 'Technical', 'chip' => 'bg-violet-50 text-violet-700 border-violet-200', 'active' => 'bg-violet-600 text-white border-violet-600 shadow-violet-600/25', 'dot' => 'bg-violet-500'],
      'general' => ['label' => 'General', 'chip' => 'bg-sky-50 text-sky-700 border-sky-200', 'active' => 'bg-sky-600 text-white border-sky-600 shadow-sky-600/25', 'dot' => 'bg-sky-500'],
    ];
  @endphp

  <div class="max-w-4xl mx-auto space-y-6" x-cloak x-data='skillForm({
    category: "{{ old('category', 'marketplace') }}",
    ...
    <div class="anim-in relative bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 rounded-2xl p-7 sm:p-8 overflow-hidden shadow-lg shadow-orange-500/25">
      ...
      <div class="relative flex items-center gap-4">
        ...
        <div>
          <h2 class="text-lg font-extrabold text-white">Tambah Skill Baru</h2>
          <p class="text-sm text-orange-100 mt-0.5">Pilih kategori, tulis nama keahlian, pratinjau langsung di kanan.</p>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-6 items-start">
      {{-- Form --}}
      <form method="POST" action="{{ route('admin.skills.store') }}" x-on:submit="submitting = true" class="xl:col-span-3 space-y-5">
        @csrf
        ...
        <div class="anim-in flex flex-col sm:flex-row gap-3 sm:justify-end" style="animation-delay:160ms">
          ...
          <button type="submit" :disabled="submitting" class="shimmer inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-amber-500 hover:bg-amber-600 active:scale-95 shadow-sm shadow-amber-500/25 transition-all duration-200 hover:-translate-y-0.5" :class="submitting ? 'opacity-70 cursor-wait' : ''">
            ...
            <span x-text="submitting ? 'Menyimpan...' : 'Simpan Skill'"></span>
          </button>
        </div>
      </form>
